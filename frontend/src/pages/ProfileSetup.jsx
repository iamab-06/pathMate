...
function ProfileSetup() {
  ...
  return (
    <div className="flex flex-col gap-6 w-full max-w-2xl mx-auto animate-fade-in">
      ...
      {/* Back Button */}
      <Link to="/dashboard" className="text-slate-500 hover:text-slate-900 flex items-center gap-2 text-sm transition-colors w-fit">
      ...
      <div className="glass-elevated rounded-3xl p-8 md:p-12 relative overflow-hidden">
        ...
        <div className="flex items-center justify-between mb-8 relative z-10">
          ...
          <div className="text-right hidden sm:block">
            <span className="text-xs text-slate-500 uppercase tracking-widest font-bold">Step 2 of 2</span>
            <div className="flex gap-1 mt-2">
              <div className="h-1 w-8 bg-cyan-500 rounded-full" />
              <div className="h-1 w-8 bg-cyan-500 rounded-full shadow-[0_0_10px_rgba(6,182,212,0.5)]" />
            </div>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6 relative z-10">
          {user?.role === 'mentee' ? (
            <>
              <div>
                <label className="block text-sm text-slate-600 mb-2 font-medium">College Tier</label>
                <select value={menteeData.college_tier} onChange={(e) => setMenteeData({ ...menteeData, college_tier: e.target.value })} required className="glass-input w-full px-4 py-3 rounded-xl text-sm appearance-none">
                  <option value="" className="bg-white text-slate-900">Select tier</option>
                  <option value="1" className="bg-white text-slate-900">Tier 1 (IIT, NIT, BITS, etc.)</option>
                  <option value="2" className="bg-white text-slate-900">Tier 2 (Good State Colleges)</option>
                  <option value="3" className="bg-white text-slate-900">Tier 3 (Local/Private Colleges)</option>
                </select>
              </div>
              <div>
                <label className="block text-sm text-slate-600 mb-2 font-medium">Target Role</label>
              ...
              <div>
                <label className="block text-sm text-slate-600 mb-2 font-medium">Current Skills</label>
            ...
            <>
              <div>
                <label className="block text-sm text-slate-600 mb-2 font-medium">Current Company</label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm text-slate-600 mb-2 font-medium">Years of Experience</label>
                ...
                <div>
                  <label className="block text-sm text-slate-600 mb-2 font-medium">Domain</label>
              ...
              <div>
                <label className="block text-sm text-slate-600 mb-2 font-medium">Your Previous College Tier</label>
              ...
              <div>
                <label className="block text-sm text-slate-600 mb-2 font-medium">Internship Status (when you started)</label>
                <select value={mentorData.internship_status} onChange={(e) => setMentorData({ ...mentorData, internship_status: e.target.value })} required className="glass-input w-full px-4 py-3 rounded-xl text-sm appearance-none">
                  ...
                  <option value="mass" className="bg-white text-slate-900">Mass recruiter internship</option>
                </select>
              </div>
            </>
          )}
