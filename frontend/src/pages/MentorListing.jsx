  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="animate-fade-in mb-12 hero-spotlight">
        <p className="text-xs font-bold text-zinc-600 uppercase tracking-widest mb-3">Explore</p>
          ...
          <div className="glass-surface rounded-2xl p-5 lg:sticky lg:top-8 shadow-2xl">
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-zinc-400 font-semibold text-xs uppercase tracking-widest flex items-center gap-2">
                <SlidersHorizontal className="w-3.5 h-3.5 text-indigo-400/70" strokeWidth={2} /> Filters
              </h2>
              {activeFilterCount > 0 && (
                <button onClick={clearFilters} className="text-xs text-zinc-600 hover:text-indigo-400 transition-colors">
                  Clear all
                </button>
              )}
            </div>
            ...
            <div className="space-y-5">
              <div>
                <label className="block text-xs text-zinc-600 mb-2 uppercase tracking-wider font-medium">College Tier</label>
                <select value={filters.previous_college_tier} onChange={(e) => handleFilterChange('previous_college_tier', e.target.value)} className="glass-input w-full px-4 py-2.5 rounded-xl text-sm appearance-none">
                  <option value="" className="bg-[#09090b] text-white">All tiers</option>
                  <option value="1" className="bg-[#09090b] text-white">Tier 1 (IIT, NIT, BITS)</option>
                  <option value="2" className="bg-[#09090b] text-white">Tier 2 (State Colleges)</option>
                  <option value="3" className="bg-[#09090b] text-white">Tier 3 (Local/Private)</option>
                </select>
              </div>

              <div>
                <label className="block text-xs text-zinc-600 mb-2 uppercase tracking-wider font-medium">Starting Point</label>
                <select value={filters.internship_status} onChange={(e) => handleFilterChange('internship_status', e.target.value)} className="glass-input w-full px-4 py-2.5 rounded-xl text-sm appearance-none">
                  <option value="" className="bg-[#09090b] text-white">Any background</option>
                  ...
                  <option value="mass" className="bg-[#09090b] text-white">Mass recruiter</option>
                </select>
              </div>

              <div>
                <label className="block text-xs text-zinc-600 mb-2 uppercase tracking-wider font-medium">Domain</label>
              ...
              <div>
                <label className="block text-xs text-zinc-600 mb-2 uppercase tracking-wider font-medium">Company</label>
        ...
        <div className="flex-1 min-w-0">
          ...
          {activeFilterCount > 0 && (
            <div className="flex flex-wrap items-center gap-2 mb-6 animate-fade-in">
              <span className="text-xs text-zinc-600 uppercase tracking-wider font-medium mr-1">Active:</span>
              {filters.previous_college_tier && (
                <span className="badge badge-neon-purple flex items-center gap-1.5">
                  ...
                  <button onClick={() => handleFilterChange('previous_college_tier', '')} className="hover:text-white transition-colors">&times;</button>
                </span>
              )}
              {filters.internship_status && (
                <span className="badge badge-neon-cyan flex items-center gap-1.5">
                  ...
                  <button onClick={() => handleFilterChange('internship_status', '')} className="hover:text-white transition-colors">&times;</button>
                </span>
              )}
              {filters.domain && (
                <span className="badge badge-emerald flex items-center gap-1.5">
                  {filters.domain}
                  <button onClick={() => handleFilterChange('domain', '')} className="hover:text-white transition-colors">&times;</button>
                </span>
              )}
              {filters.company && (
                <span className="badge badge-amber flex items-center gap-1.5">
                  {filters.company}
                  <button onClick={() => handleFilterChange('company', '')} className="hover:text-white transition-colors">&times;</button>
                </span>
              )}
            </div>
          )}
                ...
                <div key={i} className="glass-surface rounded-2xl p-6 animate-pulse">
                  <div className="flex items-start gap-4 mb-4">
                    ...
                    <div className="flex-1 mt-1">
                      <div className="h-4 bg-white/[0.06] rounded-full w-2/3 mb-3" />
                      <div className="h-3 bg-white/[0.03] rounded-full w-1/2" />
                    </div>
                  </div>
                  <div className="h-3 bg-white/[0.03] rounded-full w-full mb-5 mt-8" />
                  <div className="flex gap-2">
                    <div className="h-5 bg-white/[0.03] rounded-full w-14" />
                    <div className="h-5 bg-white/[0.03] rounded-full w-20" />
                  </div>
                </div>
              ))}
            </div>
          ...
          {!loading && !error && mentors.length === 0 && (
            <div className="glass-surface rounded-2xl p-16 text-center flex flex-col items-center">
              ...
              <h3 className="text-white font-bold text-xl mb-3 tracking-tight">No mentors found</h3>
          ...
          {!loading && !error && mentors.length > 0 && (
            <>
              <p className="text-xs text-zinc-600 mb-4 uppercase tracking-widest font-bold flex items-center gap-2">
                <Users className="w-3.5 h-3.5 text-zinc-600" strokeWidth={2} />
